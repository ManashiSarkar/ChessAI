...
{
	strcpy(board[8]+1,"rnbqkbnr");strcpy(board[7]+1,"pppppppp");
	strcpy(board[2]+1,"PPPPPPPP");strcpy(board[1]+1,"RNBQKBNR");
}
...
inline void gameOver(int draw=0)
{
	if( draw )
		cout << "Draw by repitition";
	else if( gameData.check[turn] ) 
		cout << "Check mate!";
	else 
		cout << "Stale mate!";
	exit(0);
...
inline bool invalid()
{
	if( !xy(tr,tc,fr,fc) || ( tr == fr && tc == fc ) ) { cout << "range error or same square" << endl; return true; }

	if( board[fr][fc] == 0 ) { cout << "'from' is empty" << endl; return true; }

	if( V('K',turn,tr,tc) ) { cout << "can't land on king" << endl; return true; }

	if( !landable( board[fr][fc], board[tr][tc] ) ) { cout << "not a landable square" << endl; return true; }

	if( turn == 1 && board[fr][fc] > 'a' ) { cout << "'from' is black" << endl; return true; }

	if( turn == 0 && board[fr][fc] < 'a' ) { cout << "'from' is white" << endl; return true; }
