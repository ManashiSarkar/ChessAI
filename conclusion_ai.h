...
bool underAttack(int r,int c)
{
	...
	forr(sr,-1,2) forr(sc,-1,2) forr(i,1,9) // R B Q
	{
		...
		if( xy(r2,c2) && ( sr || sc ) )
		{
			if( !(sr * sc) && ( V('Q',1-turn,r2,c2) || V('R',1-turn,r2,c2) ) ) return true;
			if( sr * sc && ( V('Q',1-turn,r2,c2) || V('B',1-turn,r2,c2) ) ) return true;
...
bool validity()
{
	...
	if( V('K',turn,r,c) )
	{
		...
		if( fr == tr && abs(fc-tc) == 2 && fc == 5 && !gameData.check[turn] ) // castling
		{
			...
			{
				if( cond3 && !underAttack(fr,fc-1) && !underAttack(fr,fc-2) ) //&& !underAttack(fr,fc-3) ) -> only king can't move through check, rook can
				{
...
bool canmove()
{
	...
	/*cout << "requestGrantedCastling : " << requestGrantedCastling << endl;
	cout << "enPass : " << enPass << endl; 
	cout << "enPassCapture : " << enPassCapture << endl;
